...
for (let i=1; i < sayilar.length; i++) {
  if (sayilar[i] < enkucuk) sayilar[i];
  if (sayilar[i] > enbuyuk) sayilar[i];
}
console.log(enkucuk);
console.log(enbuyuk);
...
console.log(ucebolunenlerintoplami0);
...
for (const sayi in tekrarSayisi) {
  if (tekrarSayisi[sayi] > 1) {
    const tekrarBilgisi = `{sayi} sayısı ${tekrarSayisi[sayi]} kere tekrar edilmiştir`;
    tekraredensayilar.push(tekrarBilgisi);
  }
}
